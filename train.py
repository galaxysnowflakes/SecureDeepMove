...
def generate_input_history(data_neural, mode, mode2=None, candidate=None):
    ...
    for u in candidate: #每次循环就是一个人的记录
        sessions = data_neural[u]['sessions']   #这个人路径记录
        train_id = data_neural[u][mode]   #是train的数据还是test的数据
        data_train[u] = {}
        for c, i in enumerate(train_id):   #遍历这个人的路径
            if mode == 'train' and c == 0:   #第一个路径不记录——不知道是为什么
                continue
            session = sessions[i]   #取这个人路径的一个路径，其实这个路径划分的依据还不知道
            trace = {}
            loc_np = np.reshape(np.array([s[0] for s in session[:-1]]), (len(session[:-1]), 1))   #取一段路径的第一个——倒数第二个的位置信息
            tim_np = np.reshape(np.array([s[1] for s in session[:-1]]), (len(session[:-1]), 1))   #取一段路径的第一个——倒数第二个的时间信息
            # voc_np = np.reshape(np.array([s[2] for s in session[:-1]]), (len(session[:-1]), 27))
            target = np.array([s[0] for s in session[1:]])  #目标是第二个——最后一个的位置信息
            trace['loc'] = Variable(torch.LongTensor(loc_np))   #都变成torch的变量
            trace['target'] = Variable(torch.LongTensor(target))
            trace['tim'] = Variable(torch.LongTensor(tim_np))
            # trace['voc'] = Variable(torch.LongTensor(voc_np))

            history = []
            if mode == 'test':   #如果是测试集，那就可以把训练部分的内容也作为历史数据添加进去
                test_id = data_neural[u]['train']
                for tt in test_id:
                    history.extend([(s[0], s[1]) for s in sessions[tt]])   #都记录在history里
            for j in range(c):
                history.extend([(s[0], s[1]) for s in sessions[train_id[j]]])
            history = sorted(history, key=lambda x: x[1], reverse=False)   #history按照时间排序
            ...
            history_loc = np.reshape(np.array([s[0] for s in history]), (len(history), 1))   #取位置信息，打成一维
            history_tim = np.reshape(np.array([s[1] for s in history]), (len(history), 1))   #取时间信息，打成一维
            trace['history_loc'] = Variable(torch.LongTensor(history_loc))
            trace['history_tim'] = Variable(torch.LongTensor(history_tim))
            if mode2 == 'avg':
                trace['history_count'] = history_count

            data_train[u][i] = trace  #最终合并记录在data_train里
        train_idx[u] = train_id  #记录的是每个数据中的train的id，每个都是一个数组，而不是一个数字
...
def generate_queue(train_idx, mode, mode2):
    ...
    user = list(train_idx.keys())  #相当于获得了train_idx中的所有的内容，对应所有用户的id ;这里加上了list，python3和2之间的不同
    train_queue = deque()  #双端队列
    if mode == 'random':  #这段还没看
        initial_queue = {}
        for u in user:
            if mode2 == 'train':
        ...
        for u in user:
            for i in train_idx[u]:
                train_queue.append((u, i))  #把路径信息都存到train_quequ里，u-用户，i-路径id
    return train_queue
...
def get_acc(target, scores):
    ...
    val, idxx = scores.data.topk(10, 1)   #这应该是返回top10的意思
...
def run_simple(data, run_idx, mode, lr, clip, model, optimizer, criterion, mode2=None):
    ...
    run_queue = None
    if mode == 'train':
        model.train(True)  #相当于启动batch normalization和drop out，只有训练过程才需要
    ...
    for c in range(queue_len):
        ...
        loss = criterion(scores, target)  #就是计算损失的常见操作
